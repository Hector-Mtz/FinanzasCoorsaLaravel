...
<template>
    <tr>
        <td class="break-all">#{{ props.deposito.nombre }}</td>
        <td>${{ formatoMoney(props.deposito.cantidad) }}</td>
        <td>
            <span v-for="factura in props.deposito.facturas" :key="factura.referencia">
                #{{ factura.referencia }}
            </span>
        ...
        <td>
            <div v-if="deposito.documento">
                <a v-if="deposito.documento.endsWith('.svg') || deposito.documento.endsWith('.png') || deposito.documento.endsWith('.pdf') || deposito.documento.endsWith('.jpg')"
                    data-fancybox :href="deposito.documento"
                ...
                <a class="inline-flex items-center justify-center px-4 py-1 text-xs font-semibold tracking-widest text-white uppercase transition bg-blue-500 border border-transparent rounded-3xl disabled:opacity-25"
                    :href="deposito.documento" download v-else>
                    <svg xmlns="http://www.w3.org/2000/svg" fill="currentColor" class="h-4" viewBox="0 0 16 16">
                        <path d="M10.5 8a2.5 2.5 0 1 1-5 0 2.5 2.5 0 0 1 5 0z" />
                        <path
                            d="M0 8s3-5.5 8-5.5S16 8 16 8s-3 5.5-8 5.5S0 8 0 8zm8 3.5a3.5 3.5 0 1 0 0-7 3.5 3.5 0 0 0 0 7z" />
        ...
        <td v-if="$page.props.can['deposito.cerrar']">
            <ButtonPres @click="emit('changeStatus', props.deposito)" class="py-1 rounded-full"> <svg
                    v-if="props.deposito.status_id === 2" xmlns="http://www.w3.org/2000/svg" class="w-4 h-4" fill="none"
                    viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                    <path stroke-linecap="round" stroke-linejoin="round"
                        d="M12 15v2m-6 4h12a2 2 0 002-2v-6a2 2 0 00-2-2H6a2 2 0 00-2 2v6a2 2 0 002 2zm10-10V7a4 4 0 00-8 0v4h8z" />

                </svg>
                <svg v-else xmlns="http://www.w3.org/2000/svg" class="w-4 h-4 text-green-500" fill="none"
                    viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                    <path stroke-linecap="round" stroke-linejoin="round"
        ...
        <td v-if="$page.props.can['deposito.delete']">
            <DangerButton @click="emit('delete', props.deposito)">
                <svg class="w-4 h-4" xmlns="http://www.w3.org/2000/svg" style="fill: white" viewBox="0 0 96 96"
